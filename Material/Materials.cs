...
public static class Materials
{
    ...
    public static MaterialId GuessFromValues(RigidBody b)
    {
        var best = MaterialId.Custom;
        float bestScore = float.MaxValue;
        foreach (var m in All)
        {
            if (m.Id == MaterialId.Custom) continue;
            float score = 0f;
            score += MathF.Abs(b.Density - m.Density) / 0.15f;
            score += MathF.Abs(b.Friction - m.Friction) / 0.20f;
            score += MathF.Abs(b.Restitution - m.Restitution) / 0.20f;
            score += MathF.Abs(b.Flammability - m.Flammability) / 0.25f;
            score += MathF.Abs(b.Conductivity - m.Conductivity) / 0.25f;
            if (b.Breakable != m.Breakable) score += 2.5f;
            score += MathF.Abs(b.BreakThreshold - m.BreakThreshold) / 4.0f;
            if (score < bestScore)
            {
                bestScore = score;
                best = m.Id;
